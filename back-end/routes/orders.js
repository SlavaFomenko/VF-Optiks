...
router.post('', async (req, res) => {
    try {
        const { customer_id, delivery_type_id = null, order_details, address = null} = req.body;
        ...
        if (!customer_id || delivery_type_id || !order_details || order_details.length === 0 || address) {
            return res.status(400).json({ error: 'Invalid request. Please provide required parameters.1' });
        }
...
router.get('/:order_id?' ,async (req , res)=>{
    try {
        let query = knexDB('Orders')
            ...
            .leftJoin('DeliveryTypes', 'DeliveryTypes.delivery_type_id', '=', 'Orders.delivery_type_id')
